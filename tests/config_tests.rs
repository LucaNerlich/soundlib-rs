...
use common::{write_config, TestLibrary};
...
fn load_uses_yaml_and_env_overrides() {
    ...
    write_config(config_file.path(), &PathBuf::from("/overridden/by/env"));
...
fn config_roundtrips_through_yaml() {
    ...
    write_config(config_file.path(), &lib.root);
    ...
    with_vars(
        ...
        || {
            ...
            assert_eq!(again.volume, loaded.volume);
        },
    );
}
